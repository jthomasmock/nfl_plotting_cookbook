...
We'll create a function that we apply across multiple inputs. Otherwise we'd need to call the scraping function 14 separate times!
  
```{r}
# Weekly NFL ratings by ESPN Scrape Function ------------------------------

qbr_weekly <- function(week){
  
  # Be respectful - always pause between scraping sessions
  Sys.sleep(5)
  
  # Build up URL
  url <- glue::glue("http://www.espn.com/nfl/qbr/_/year/2019/type/player-week/week/{week}")
...
raw_qbr <- 1:14 %>% 
  map_dfr(.f = qbr_weekly) %>% 
  mutate_at(.vars = vars(rk, pts_added:week), as.double) %>% 
  separate(player, into = c("player", "team_abb"), sep = ",") %>% 
  mutate_at(.vars = vars(player, team_abb), str_trim)
...
all_qbr <- raw_qbr %>% 
  ...
  left_join(df_logos, by = c("team_abb" = "team_code")) %>% 
  group_by(week) %>% 
  mutate(rk = row_number()) %>% 
  ungroup() %>% 
  left_join(nfl_colors, by = "team") 
```
...
average_qbr <- all_qbr %>% 
  group_by(player) %>% 
  mutate(avg_qbr = cummean(total_qbr)) %>% 
  ungroup()
...
weekly_plot <- average_qbr %>% 
  group_by(week) %>% 
  arrange(desc(avg_qbr)) %>% 
  mutate(rk = row_number()) %>% 
  ungroup() %>% 
  ggplot(aes(x = rk, y = avg_qbr)) +
  geom_col(aes(fill = primary, color = primary), alpha = 0.7) +
