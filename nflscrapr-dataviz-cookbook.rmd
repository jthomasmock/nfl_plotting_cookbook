...
This allows you to specify colors of interest
...
This allows you to assign multiple evaluation steps
...
- A) use a built in theme (theme_bw, theme_minimal, etc), 
- B) a pre-built theme (bbplot, ggthemes, etc)
- C) Or build your own theme!  

All are valid, but you don't necessarily have to actually manually code the theme element changes to each and every plot. You can at the least write your own theme as a function and use it. Alternatively, you can write your own package (scarier than it sounds!) and source that.
...
qbs %>%
  ...
  geom_text_repel(data = filter(qbs, posteam == "SF"),
...
qbs %>%
  ...
  geom_mark_hull(aes(filter = name %in% c("J.Garoppolo", "N.Mullens"), description = "Mullens + Garoppolo performed similarly in 2018"), color = "red",
                 label.fontface = "bold", label.colour = "red", con.colour = "red") +
  labs(x = "Success rate",
       y = "EPA per play",
       caption = "Data from nflscrapR",
       title = "QB success rate and EPA/play",
...
schotty_gt %>% 
  ...
  tab_header(title = "Percentage of Passes by teams on 1st/2nd Down in 1st Half",
...
rbs <- c("A.Kamara", "J.White", "J.Conner", "C.McCaffrey", "S.Barkley", "E.Elliott",
         "J.Mixon", "T.Gurley", "D.Johnson", "M.Gordon")

wrs <- c("D.Westbrook", "A.Humphries", "C.Kupp", "G.Tate", "D.Pettis", "J.Edelman",
