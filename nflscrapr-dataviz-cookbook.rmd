...
```{r}
data_clean <- pbp_rp %>%  filter(pass == 1 & sack == 0 & qb_scramble == 0) %>%
  select(
...
problem_wrs <- c("K.Benjamin", "A.Cooper", "G.Tate", "A.Robinson", "B.Marshall",
...
problem_rbs <- c("A.Abdullah", "C.Hyde", "Dam.", "T.Montgomery", "A.Ekeler", "T.Yeldon",
...
pos %>% 
  ...
  theme(legend.position = "none") +
  scale_fill_manual(values = c("#ff2b4f", "#00b159", "#003399"),
                    aesthetics = c("color", "fill")) +
  labs(x = "Air Yards (Depth of Target)",
       y = "EPA\n",
...
Speaking of distributions - let's jump into the next section!
